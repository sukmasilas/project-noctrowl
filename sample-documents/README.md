...
- **Category data — confirmed absent.** No category ID or item specifics column anywhere in the export. Only `Item title` (free text) and `Custom label` (SKU field — see consignment note below). Item titles alone span what look like TCG cards, watches (Seiko), *and* dolls/Hot Wheels — categories outside the declared TCG/Watches/Auto parts three, worth a quick sanity check with you on whether that's a real 4th line of business or just how this particular month's inventory reads. Either way: **category tagging cannot be pulled automatically from this export** — confirms the open question in CLAUDE.md. It'd have to wait for API sync (deferred) or a different report type, if one exists that carries category data.
