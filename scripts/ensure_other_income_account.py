...
(consolidated singleton, IDR) for the new OTHER_INCOME account type on the
already-seeded real ``noctrowl`` database.

WHY THIS IS A SCRIPT, NOT A ``ledger.migrations`` STEP: exactly the same
reasoning as ``scripts/ensure_contract_labor_account.py`` (see that script's
own docstring) — ``ledger.migrations``'s ``account_types_other_income`` step
only creates the chart-of-accounts CATALOG row. The actual ``accounts`` row
...
USAGE:
    python3 scripts/ensure_other_income_account.py
...
ACCOUNT_TYPE_CODE = "OTHER_INCOME"
