...
(consolidated singleton, IDR) for the new STAFF_MEALS_WELFARE operating
-expense account type on the already-seeded real ``noctrowl`` database.

Same exact pattern/reasoning as ``scripts/ensure_packaging_supplies_account.py``
...
``account_types_staff_meals_welfare``), but the actual postable ``accounts``
...
USAGE:
    python3 scripts/ensure_staff_meals_welfare_account.py
...
ACCOUNT_TYPE_CODE = "STAFF_MEALS_WELFARE"
