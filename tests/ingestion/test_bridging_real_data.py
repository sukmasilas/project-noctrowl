...
def test_real_four_month_bridging_and_main_data_no_phantom_double_post(iprototype):
    ...
    # entries (no double-post, none missing). ---
    ...
    assert len(all_entries) == 23  # nothing else got posted (no invoices/consignment/keyword fixtures loaded)
    ...
    # --- 6. Every OTHER real Bridging/Main line (fees, interest, tax,
    ...
    # falls to Needs Review (no invoice/consignment/keyword fixtures are
    # loaded in this test, so nothing else COULD legitimately auto-match).
    needs_review_descriptions = conn.execute(
        select(review_queue.c.raw_description).where(review_queue.c.category.is_(None))
    ).scalars().all()
    assert len(needs_review_descriptions) == staged_bridging + staged_main - len(landing_rows) - len(sweep_rows)
    assert any("Bunga rekening" in d for d in needs_review_descriptions)  # interest — not dropped, not misclassified
    assert any("Pajak rekening" in d for d in needs_review_descriptions)  # tax on interest
    assert any("Biaya administrasi" in d for d in needs_review_descriptions)  # admin fees
    ...
    assert len(entries_after_rerun) == 23  # still exactly 23, not 46
