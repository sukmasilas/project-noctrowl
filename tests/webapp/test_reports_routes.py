...
def test_revenue_report_renders_empty_period(client, wtopology):
    resp = client.get("/reports/revenue?period=2026-07")
    assert resp.status_code == 200
    assert b"Provisional" in resp.data  # nothing ingested yet for this period


def test_revenue_report_renders_with_data(client, wtopology):
    conn, topo = wtopology
    ...
    conn.commit()
    resp = client.get(f"/reports/revenue?period=2026-07&account_id={topo['ebay_account_id']}")
    assert resp.status_code == 200


def test_cash_flow_report_renders(client, wtopology):
    resp = client.get("/reports/cash-flow?period=2026-07")
    assert resp.status_code == 200
    assert b"Statement of Cash Flows" in resp.data


def test_cash_flow_report_is_consolidated_only(client, wtopology):
    resp = client.get("/reports/cash-flow?period=2026-07")
    assert resp.status_code == 200
    assert b"disabled" in resp.data  # account selector locked out, same as P&L/Equity/Balance Sheet


def test_cash_flow_drilldown_route_renders(client, wtopology):
    conn, topo = wtopology
    posting.post_ebay_sale(
    ...
    conn.commit()
    resp = client.get("/reports/cash-flow/drilldown/cash_from_customers?period=2026-07")
    assert resp.status_code == 200
    assert b"CF-DD" in resp.data


def test_cash_flow_drilldown_unknown_key_404s(client, wtopology):
    resp = client.get("/reports/cash-flow/drilldown/not_a_real_key?period=2026-07")
    assert resp.status_code == 404


def test_pnl_report_renders_and_is_consolidated_only(client, wtopology):
    resp = client.get("/reports/pnl?period=2026-07")
    assert resp.status_code == 200
    assert b"disabled" in resp.data  # account selector locked out


def test_equity_report_renders(client, wtopology):
    resp = client.get("/reports/equity?period=2026-07")
    assert resp.status_code == 200


def test_drilldown_route_renders_matching_lines(client, wtopology):
    conn, topo = wtopology
    posting.post_ebay_sale(
        conn, ebay_account_id=topo["ebay_account_id"], entry_date=DAY, gross_sale_price_usd=Decimal("50"),
    ...
    conn.commit()
    resp = client.get(f"/reports/drilldown/sales_revenue?period=2026-07&account_id={topo['ebay_account_id']}")
    assert resp.status_code == 200
    assert b"R2" in resp.data


def test_drilldown_unknown_figure_404s(client, wtopology):
    resp = client.get("/reports/drilldown/not_a_real_figure?period=2026-07")
    assert resp.status_code == 404
...
def test_balance_sheet_renders_with_data_and_balances(client, wtopology):
    conn, topo = wtopology
...
def test_balance_sheet_account_drilldown_route_renders(client, wtopology):
    conn, topo = wtopology
    posting.post_owner_contribution(conn, entry_date=DAY, amount_idr=Decimal("10000000"))
    conn.commit()
    ...
    resp = client.get(f"/reports/balance-sheet/drilldown/{bca_main.account_id}?period=2026-07")
    assert resp.status_code == 200
