"""Tests for the General Ledger screen (webapp/general_ledger_bp.py) — the
posted-only, whole-chart-of-accounts, double-entry browse view. See
CLAUDE.md's General Ledger brief (Phase 2 of the UI project) and
...
from webapp.general_ledger_bp import general_ledger_entries, list_all_accounts
...
def test_general_ledger_all_accounts_mode_shows_both_lines_of_each_entry(wtopology):
    conn, topo = wtopology
...
def test_general_ledger_all_accounts_mode_is_scoped_to_the_selected_period_only(wtopology):
    conn, topo = wtopology
    ...
    july_entries = general_ledger_entries(
        conn, account_option=None, period_month=_dt.date(2026, 7, 1), scope_all=False
    )
    august_entries = general_ledger_entries(
...
def test_general_ledger_account_filter_only_shows_entries_touching_that_account(wtopology):
    conn, topo = wtopology
    post_cogs_purchase(conn, entry_date=_dt.date(2026, 7, 10), amount_idr=Decimal("150000"))
...
def test_general_ledger_account_filter_computes_running_balance_via_shared_helper(wtopology):
    conn, topo = wtopology
...
def test_general_ledger_account_filter_all_periods_scope_spans_multiple_months(wtopology):
    conn, topo = wtopology
    ...
    period_only = general_ledger_entries(
        conn, account_option=cogs_opt, period_month=_dt.date(2026, 7, 1), scope_all=False
    )
    all_time = general_ledger_entries(
...
def test_general_ledger_reversal_pair_is_visibly_linked_both_directions(wtopology):
    conn, topo = wtopology
...
def test_general_ledger_real_reversal_pairs_from_actual_dev_data_shape(wtopology):
...
def test_general_ledger_traces_review_queue_and_invoice_source(wtopology):
    conn, topo = wtopology
...
def test_general_ledger_entry_with_no_linked_source_shows_empty_trace_not_a_crash(wtopology):
    conn, topo = wtopology
...
def test_general_ledger_route_renders_reversal_badges_for_real_shape(logged_in_client, wtopology):
    conn, topo = wtopology
    original_id = post_cogs_purchase(conn, entry_date=_dt.date(2026, 7, 10), amount_idr=Decimal("50000"))
    reversal_id = post_reversal_entry(
        conn,
...
def test_general_ledger_owner_capital_line_shows_income_posting(wtopology):
    conn, topo = wtopology
