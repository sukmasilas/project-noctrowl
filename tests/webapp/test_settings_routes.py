...
def test_payout_tiers_page_lists_all_rows(client, wtopology):
    resp = client.get("/settings/payout-tiers")
    assert resp.status_code == 200
    assert b"72.00" in resp.data or b"72" in resp.data


def test_update_rate_changes_only_that_row(client, wtopology):
    conn, topo = wtopology
...
def test_manual_contact_tier_cannot_be_edited(client, wtopology):
    conn, topo = wtopology
...
def test_invalid_rate_rejected(client, wtopology):
    conn, topo = wtopology
    first_row = conn.execute(
        select(consignor_payout_tiers).where(consignor_payout_tiers.c.display_order == 1)
    ).first()
    ...
    resp = client.post(f"/settings/payout-tiers/{first_row.id}", data={"rate_percent": "not-a-number"})
    assert resp.status_code in (301, 302)
