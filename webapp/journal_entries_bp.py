"""Journal Entries screen: the formal, posted-only, double-entry accounting
...
Renamed from "General Ledger" (2026-09-16) to reflect what this screen has
always actually been: the chronological book of original entry, showing
every posted journal entry in full with BOTH its debit and credit lines
together. That's the accounting definition of a Journal, not a General
Ledger — a real General Ledger is organized BY ACCOUNT, one row per journal
LINE touching that account, with a running balance, and does NOT show the
other side of the entry inline (see webapp/general_ledger_bp.py, the new
screen that now actually implements that view). This module's own
behavior/logic is otherwise unchanged by the rename — filtering by account
here is still "show me every full entry that touched this account," not a
per-account ledger.
...
- Default view is scoped to ONE selected period (``webapp.scoping.
  ...
  Reports/Wallet/Bank Reconciliation), not "every entry ever" — with ~950 real rows
...
def list_all_accounts(conn: Connection) -> list[AccountOption]:
    ...
    generalizes), for the Journal Entries account filter dropdown. Also
    reused as-is by webapp.general_ledger_bp for its mandatory account
    selector, since both screens need the exact same "every real,
    individually-postable account" listing.
    """
    rows = conn.execute(
        select(
